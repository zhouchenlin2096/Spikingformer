...
$E_{Spikingformer}=E_{A C} \times\left(\sum_{i=2}^N S O P_{\operatorname{Conv} }^i+\sum_{j=1}^M S O P_{\operatorname{SSA}}^j\right)+E_{M A C} \times\left(FLOP_{\operatorname{Conv}}^1\right) \label{eq:energy1}$
